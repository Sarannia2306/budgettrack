...
class _AddIncomeScreenState extends State<AddIncomeScreen> {
  ...
  List<String> categories = ['Salary', 'Investments', 'Freelance'];
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Income Title
            _buildTextField(titleController, 'Income Title'),
            SizedBox(height: 20),
  ...
  Widget _buildAddButton(String buttonText) {
    return Container(
      ...
      child: ElevatedButton(
        onPressed: _saveIncomeToFirebase,
        style: ElevatedButton.styleFrom(
  ...
  void _saveIncomeToFirebase() async {
    if (_user == null) {
    ...
    if (titleController.text.isNotEmpty && amountController.text.isNotEmpty && selectedCategory.isNotEmpty) {
      ...
      // Reference to the user's transactions/income in Firebase
      DatabaseReference userRef = _database.ref('users/${_user?.uid}/transactions/income');
      ...
      try {
        // Push new income data to Firebase
        await userRef.push().set(newIncome);

        // Add notification
        _addNotification("Successfully added income of RM $amount for $title on $time", 'income');
